...
function LineGraph(argsMap) {
	...
	var _init = function() {
		...
		margin[2] = getOptionalVar(argsMap, 'marginBottom', 30) // marginBottom allows fitting the legend along the bottom
		margin[3] = getOptionalVar(argsMap, 'marginLeft', 50) // marginLeft allows fitting the axis labels
	...
	*/
	var createGraph = function() {
		...
		lineFunction = d3.svg.line()
			...
			.x(function(d,i) { 
				 ...
				 */
				var _x = x(data.startTime + (data.step*i)); 
	...
	var handleMouseClick = function(event) {	
		...
		if(mouseX >= 0 && mouseX <= w && mouseY >= 0 && mouseY <= h) {
			...
			userCurrentlyInteracting = true;
			currentUserPositionX = mouseX;
			console.info("clicked at "+currentUserPositionX);
		} else {
			// proactively act as if we've left the area since we're out of the bounds we want
			handleMouseOutGraph(event)
		}
...
*/
	...
	*/
	var getValueForPositionXFromData = function(xPosition, dataSeriesIndex) {
		...
		var index = (xValue - data.startTime) / data.step;
		...
		var bucketDate = data.startTime + data.step * (index+1); // index+1 as it is 0 based but we need 1-based for this math
