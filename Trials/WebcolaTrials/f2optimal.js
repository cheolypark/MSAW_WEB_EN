...
{
    data["inputs"]=[];
    var displayCases=document.getElementById("displayCases").value;
    ...
    if(data["outputAttribute"]==null)
    {
        ...
        if(displayCases=="" || totalCases=="")
            ...
            data["inputs"].push({"totalCases": totalCases});
            data["inputs"].push({"displayCases": displayCases});
...
function getCaseData(caseId) {
    d3.json("http://localhost:9019/aakarshika/sens/"+caseId,function(err, j) {          
        insideNodes=j.nodes;
        linksData = j.links;
